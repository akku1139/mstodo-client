...
export function TodoApp({ account, msalInstance, onLogout }: TodoAppProps) {
  ...
  const handleCreateTask = async (e: React.FormEvent) => {
    ...
    try {
      const task = await api.createTask(selectedListId, newTaskTitle.trim());
      setTasks([task, ...tasks]);
      setNewTaskTitle('');
    } catch (err) {
            ...
            <div className="p-2">
              {lists.map(list => (
                ...
                >
                  ...
                  <ChevronRight className={`w-4 h-4 transition-transform ${selectedListId === list.id ? 'rotate-90' : ''}`} />
                </div>
              ))}
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
              <div className="p-4 border-b border-gray-100">
                <div className="flex items-center justify-between">
                  <div>
                    <h2 className="text-xl font-bold text-gray-900">{selectedList.displayName}</h2>
...
function TaskItem({ task, onToggle, onDelete }: { task: TodoTask; onToggle: () => void; onDelete: () => void }) {
